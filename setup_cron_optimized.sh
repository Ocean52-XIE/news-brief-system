#!/bin/bash
# 优化版定时任务设置脚本

echo "=== 新闻简报定时任务设置 ==="
echo "按照建议的时间配置："
echo "1. 早间简报: 09:00"
echo "2. 午间更新: 15:00" 
echo "3. 晚间总结: 21:00"
echo ""

# 获取当前工作目录
WORKDIR=$(pwd)
echo "工作目录: $WORKDIR"

# 检查Python脚本是否存在
if [ ! -f "generate_optimized_news_brief.py" ]; then
    echo "❌ 错误: generate_optimized_news_brief.py 不存在"
    exit 1
fi

echo "✓ 找到生成脚本: generate_optimized_news_brief.py"

# 创建日志目录
LOGDIR="$WORKDIR/logs"
mkdir -p "$LOGDIR"
echo "✓ 创建日志目录: $LOGDIR"

# 生成crontab配置
CRON_CONFIG="# 新闻简报定时任务 - 优化版
# 每天上午9:00生成早间简报
0 9 * * * cd $WORKDIR && /usr/bin/python3 generate_optimized_news_brief.py >> $LOGDIR/morning_$(date +\%Y\%m\%d).log 2>&1

# 每天下午3:00生成午间更新
0 15 * * * cd $WORKDIR && /usr/bin/python3 generate_optimized_news_brief.py >> $LOGDIR/afternoon_$(date +\%Y\%m\%d).log 2>&1

# 每天晚上9:00生成晚间总结
0 21 * * * cd $WORKDIR && /usr/bin/python3 generate_optimized_news_brief.py >> $LOGDIR/evening_$(date +\%Y\%m\%d).log 2>&1

# 每周五下午6:00生成周度总结（预留）
0 18 * * 5 cd $WORKDIR && echo '周度总结功能待实现' >> $LOGDIR/weekly_$(date +\%Y\%m\%d).log 2>&1"

echo ""
echo "=== 生成的crontab配置 ==="
echo "$CRON_CONFIG"

echo ""
echo "=== 安装选项 ==="
echo "1. 自动安装到当前用户的crontab"
echo "2. 显示配置，手动安装"
echo "3. 保存配置到文件"
read -p "请选择 (1/2/3): " choice

case $choice in
    1)
        # 备份现有crontab
        crontab -l > /tmp/crontab_backup_$(date +%Y%m%d_%H%M%S).bak 2>/dev/null
        echo "✓ 已备份现有crontab"
        
        # 添加新配置
        (crontab -l 2>/dev/null; echo "$CRON_CONFIG") | crontab -
        echo "✓ 定时任务已安装"
        
        # 显示当前crontab
        echo ""
        echo "=== 当前crontab ==="
        crontab -l
        ;;
    2)
        echo ""
        echo "=== 手动安装说明 ==="
        echo "1. 运行: crontab -e"
        echo "2. 添加以下内容:"
        echo ""
        echo "$CRON_CONFIG"
        echo ""
        echo "3. 保存并退出"
        ;;
    3)
        CRON_FILE="$WORKDIR/crontab_config.txt"
        echo "$CRON_CONFIG" > "$CRON_FILE"
        echo "✓ 配置已保存到: $CRON_FILE"
        ;;
    *)
        echo "❌ 无效选择"
        exit 1
        ;;
esac

echo ""
echo "=== 测试命令 ==="
echo "手动测试生成简报:"
echo "  cd $WORKDIR && python3 generate_optimized_news_brief.py"
echo ""
echo "查看日志:"
echo "  tail -f $LOGDIR/*.log"
echo ""
echo "=== 完成 ==="